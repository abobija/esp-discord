...
#define DISCORD_PERMISSION_ADMINISTRATOR                                                                               \
    0x00000008ULL /*!< Allows all permissions and bypasses channel permission overwrites */
...
#define DISCORD_PERMISSION_VIEW_CHANNEL                                                                                \
    0x00000400ULL /*!< Allows guild members to view a channel, which includes reading messages in text channels */
...
#define DISCORD_PERMISSION_EMBED_LINKS                                                                                 \
    0x00004000ULL /*!< Links sent by users with this permission will be auto-embedded */
...
#define DISCORD_PERMISSION_MENTION_EVERYONE                                                                            \
    0x00020000ULL /*!< Allows for using the @everyone tag to notify all users in a channel, and the @here tag to       \
                     notify all online users in a channel */
#define DISCORD_PERMISSION_USE_EXTERNAL_EMOJIS                                                                         \
    0x00040000ULL /*!< Allows the usage of custom emojis from other servers */
...
typedef enum
{
    ...
    DISCORD_CLOSEOP_UNKNOWN_OPCODE, /*!< You sent an invalid Gateway opcode or an invalid payload for an opcode. Don't
                                       do that! */
    ...
    DISCORD_CLOSEOP_INVALID_SEQ =
        4007, /*!< The sequence sent when resuming the session was invalid. Reconnect and start a new session. */
    DISCORD_CLOSEOP_RATE_LIMITED, /*!< Woah nelly! You're sending payloads to us too quickly. Slow it down! You will be
                                     disconnected on receiving this. */
    ...
    DISCORD_CLOSEOP_SHARDING_REQUIRED,   /*!< The session would have handled too many guilds - you are required to shard
                                            your connection in order to connect. */
    DISCORD_CLOSEOP_INVALID_API_VERSION, /*!< You sent an invalid version for the gateway. */
    DISCORD_CLOSEOP_INVALID_INTENTS,     /*!< You sent an invalid intent for a Gateway Intent. You may have incorrectly
                                            calculated the bitwise value. */
    DISCORD_CLOSEOP_DISALLOWED_INTENTS   /*!< You sent a disallowed intent for a Gateway Intent. You may have tried to
...
typedef enum
{
    ...
    DISCORD_EVENT_DISCONNECTED, /*<! Bot is fully disconnected from discord. It's not going to automatically reconnect.
                                   To connect again, discord_login function needs to be called */
...
esp_err_t discord_register_events(
    discord_handle_t client, discord_event_t event, esp_event_handler_t event_handler, void *event_handler_arg);
 ...
 * @return number of miliseconds since esp_timer_init was called (this normally happens early during application
 * startup)
 */
