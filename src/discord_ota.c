...
static const char *discord_ota_err_string[] = { DCOTA_FOREACH_ERR(DCOTA_GENERATE_STRING) };
...
esp_err_t discord_ota_init(discord_handle_t client, discord_ota_config_t *config)
{
    ...
    discord_ota_handle_t ota =
        cu_tctor(discord_ota_handle_t, struct discord_ota, .config = cu_ctor(discord_ota_config_t));
    ...
    if (config) {
        ...
        if (config->channel) {
            ota->config->channel =
                cu_ctor(discord_channel_t, .id = STRDUP(config->channel->id), .name = STRDUP(config->channel->name));
        }
    }
    ...
    if (ota->config->multiple_ota) {
        ...
        if ((err = discord_register_events(client, DISCORD_EVENT_CONNECTED, ota_on_connected, client)) != ESP_OK
            || (err = discord_register_events(client, DISCORD_EVENT_DISCONNECTED, ota_on_disconnected, client))
                   != ESP_OK) {
            discord_ota_destroy(client);
            return err;
        }
...
static bool ota_image(discord_download_info_t *file, discord_ota_handle_t ota_hndl)
{
    ...
    if (ota_hndl->buffer_offset > sizeof(esp_image_header_t) + sizeof(esp_image_segment_header_t)
                                      + sizeof(esp_app_desc_t)) { // Image header is in buffer
        esp_app_desc_t new_app;
        memcpy(&new_app,
            ota_hndl->buffer + sizeof(esp_image_header_t) + sizeof(esp_image_segment_header_t),
        ...
        if (last_invalid_partition != NULL
            && esp_ota_get_partition_description(last_invalid_partition, &last_invalid_app) != ESP_OK) {
...
static esp_err_t discord_ota_perform(discord_handle_t client, discord_message_t *firmware_message)
{
    ...
    if (ota->config->channel) {
        ...
        int channels_len = 0;
        if ((err = discord_guild_get_channels(client,
        ...
        discord_channel_t *channel =
            discord_channel_get_from_array_by_name(channels, channels_len, ota->config->channel->name);
...
_channel_ok:
    ...
    if (!ota->config->administrator_only_disabled) {
        ...
        if ((err = discord_member_has_permissions(client,
    ...
    if (estr_sw(subcmd, "status")) {
        ...
        discord_message_send(client,
            &(discord_message_t) { .channel_id = firmware_message->channel_id, .content = msg_content },
            NULL);
...
_ota_update:
    ...
    if ((err = discord_message_download_attachment(client, firmware_message, 0, &ota_download_handler, ota))
        != ESP_OK) {
    ...
    if (!ota->config->success_feedback_disabled) {
        ...
        discord_message_send(client,
            &(discord_message_t) { .channel_id = firmware_message->channel_id, .content = success_content },
            NULL);
...
_error:
    ...
    if (!quiet && !ota->config->error_feedback_disabled) {
        ...
        discord_message_send(client,
            &(discord_message_t) { .channel_id = firmware_message->channel_id, .content = err_content },
            NULL);
        free(err_content);
    }
_return:
...
esp_err_t discord_ota_keep(bool keep_or_rollback)
{
    ...
    DISCORD_LOGW("New firmware version is going to be %s", keep_or_rollback ? "preserved" : "rolled back");
...
static discord_ota_token_t discord_ota_get_token()
{
    discord_ota_token_t token = { .err = ESP_OK };
    ...
    DISCORD_LOGI("Token loaded from %s", token.type == DISCORD_OTA_TOKEN_FROM_NVS ? "NVS" : "Configuration");
